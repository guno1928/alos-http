...
// replaceReadRing closes the ring at idx and installs a fresh one. It is the
// guaranteed-clean fallback when reclaimAfterTimeout could not drain a
// timed-out op's stale completion (e.g. kernels without IORING_REGISTER_SYNC_CANCEL):
// closing the old ring's fd makes the kernel drop its in-flight request, so no
// stale completion can be reaped by the next borrower. The caller must hold
// readMu[idx]. On allocation failure the old ring is kept (best effort) rather
// than left nil.
