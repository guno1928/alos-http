...
// having no message body per RFC 9112 §6.3: any response to a HEAD request,
// all 1xx informational responses, and 204 No Content / 304 Not Modified.
...
func (pe *ProxyEngine) forwardRequest(req *Request, resp *Response, b *backend, cfg *DomainConfig) error {
	...
	*bp = buf[:0]
	...
	// Transfer-Encoding is present (those describe the body a GET would
	// return). Attempting to read a body here blocks until ReadTimeout and
	// then discards an otherwise-reusable upstream connection. Emit the
	// headers and return without touching the body. Such responses have no
	// representation body to cache, so the cache is intentionally skipped.
