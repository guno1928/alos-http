...
// proxyRingReclaimTimeout bounds how long reclaimAfterTimeout waits for a
// cancelled operation's completion to post before signalling the caller to
// replace the ring. Only the (rare) deadline-exceeded path hits this.
const proxyRingReclaimTimeout = 250 * time.Millisecond

// reclaimAfterTimeout makes a pooled ring safe to reuse after one of its
// operations exceeded its deadline. The timed-out submitAndAwait left an op in
// flight without consuming its completion; because pooled rings do not match
// completions by user_data, the next connection to borrow the ring would reap
// that stale completion as its own (wrong byte count / queue desync).
//
// It synchronously cancels any op still attached to fd (on kernels with
...
// could not confirm cleanliness within proxyRingReclaimTimeout — in which case
// the caller must replace the ring. The wait is bounded so a stuck ring cannot
// block the proxy.
