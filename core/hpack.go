...
// on any RFC 7541 §5.2 violation: an embedded EOS symbol, padding longer than
// 7 bits, or trailing padding bits that are not all ones (the EOS prefix).
// Callers must treat ok=false as a COMPRESSION error and never use the value.
